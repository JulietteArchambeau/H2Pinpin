...
```{r CreateColumnsCTD}
df <- df %>% 
  dplyr::mutate(CTD_asturias_EMT=(invEMT-(-1)*(df.site[df.site$site=="asturias","EMT_site"] %>% pull())) %>% abs(),
                CTD_asturias_SHM=(SHM-(df.site[df.site$site=="asturias","SHM_site"] %>% pull())) %>% abs(),
                CTD_bordeaux_EMT=(invEMT-(-1)*(df.site[df.site$site=="bordeaux","EMT_site"] %>% pull())) %>% abs(),
                CTD_bordeaux_SHM=(SHM-(df.site[df.site$site=="bordeaux","SHM_site"] %>% pull())) %>% abs(),
                CTD_portugal_EMT=(invEMT-(-1)*(df.site[df.site$site=="portugal","EMT_site"] %>% pull())) %>% abs(),
                CTD_portugal_SHM=(SHM-(df.site[df.site$site=="portugal","SHM_site"] %>% pull())) %>% abs())

df %>% 
  dplyr::select(contains("EMT"),contains("SHM"),-invEMT) %>% 
  cor() %>% 
  lower.triangle() %>%
  kable(digits=3) %>%
  kable_styling(font_size=11,
