...
To obtain the climate harhness indexes, we calculated the mean over of period 1901-1950 of following annual variables:
  ...
  - $MSP$ is the mean summer precipitation.
...
The climatic data were extracted from raster files at 1kmx1km spatial resolution (kindly provided by Maurizio Marchi), in which each variable was averaged over the period 1901-1950. Here is the list of the climatic variables used:
  ...
  - Annual variables 
      ...
      - $MSP$: mean summer precipitation (mm) (named $PRCsum$ in the raster files)
...
climDT <- read.csv("data/Climate/Raw/ClimateDTdownloads/ClimateDT_outputs.csv") %>% 
  ...
  dplyr::summarise(mean_bio6=mean(bio6),   # minimum temperature of the coldest month
                   ...
                   mean_MSP=mean(MSP),     # mean summer precipitation
...
# Export drivers and save table for Supp Info
...
df.suppinfo <- df %>% 
  dplyr::select(prov, SH.20km.PC1,SH.20km.PC2, SH.1km.PC1,SH.1km.PC2,Nb.val.20km,Nb.val.1km) # Add temporal heterogeneity indexes + climate harshness indexes here ! 
...
df %>% dplyr::select(A,D,SH.20km.PC1,SH.20km.PC2,
                     SH.1km.PC1,SH.1km.PC2,SHM,invEMT) %>% 
  cor() %>% 
  lower.triangle() %>% 
  kable(digits=3) %>%
  kable_styling(font_size=11,
...
readRDS(file="data/ClonapinData/PhenoDataNovember2019_AnnualTraits.rds") %>% 
  ...
  dplyr::select(A,D,SH.20km.PC1,SH.20km.PC2,
                SH.1km.PC1,SH.1km.PC2,SHM,invEMT,altitude_prov) %>% 
  cor() %>% 
  upper.triangle() %>% 
  as.data.frame() %>% 
