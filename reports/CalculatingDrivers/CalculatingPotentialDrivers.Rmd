...
```{r}
climDTinit <- read_csv("data/Climate/Raw/ClimateDTdownloads/InitialElevationData/ClimateDT_outputs.csv") %>% 
  group_by(ID) %>% 
...
```{r}
climDTdeem <- read_csv("data/Climate/Raw/ClimateDTdownloads/DEEMElevationData/ClimateDT_outputs.csv") %>% 
  group_by(ID) %>% 
  dplyr::summarise(mean_bio6=mean(bio6),
            mean_SHM=mean(SHM),
            mean_EMT=mean(EMT),
