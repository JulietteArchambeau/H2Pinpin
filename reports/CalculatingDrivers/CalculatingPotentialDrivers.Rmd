...
# We prepare two datasets to extract the climatic variables from the climate downscaling tool

# A first dataset with the original elevation values of the populations
# Provenance coordinates
...
# and the MAD population because it has only one clone (so the genetic variation can't
# be estimated)
provcoord <- provcoord[!(provcoord$prov=="ROD"|provcoord$prov=="MAD"),]
colnames(provcoord) <- c("prov","longitude","latitude","altitude")

write_csv(provcoord,"data/Climate/Raw/ClimateDTdownloads/InitialElevationData/provenance_coordinates_readyforClimateDT.csv")



# A second dataset with the elevation values based on the DEEM values of the climateDT
provcoord <- read_csv("data/Climate/Raw/ClimateDTdownloads/InitialElevationData/ClimateDT_report.csv") %>% 
  dplyr::select(ID, Latitude,Longitude,RefElev1kmFORdownscaling) %>% 
  write_csv("data/Climate/Raw/ClimateDTdownloads/DEEMElevationData/provenance_coordinates_readyforClimateDT.csv")
```
...
## New indices of climate harshness after review

```{r}
climDTinit <- read.csv("data/Climate/Raw/ClimateDTdownloads/InitialElevationData/ClimateDT_outputs.csv") %>% 
  group_by(ID) %>% 
  dplyr::summarise(mean_bio6=mean(bio6),
            mean_SHM=mean(SHM),
            mean_EMT=mean(EMT),
            min_bio6=min(bio6),
            max_SHM=max(SHM),
  ...
  merge(df[,c("prov","EMT","SHM")],by="prov")

# vizualisation
climDTinit %>% pairs.panels(scale=T,hist.col="palegreen1")
```


```{r}
climDTdeem <- read.csv("data/Climate/Raw/ClimateDTdownloads/DEEMElevationData/ClimateDT_outputs.csv") %>% 
  group_by(ID) %>% 
  dplyr::summarise(mean_bio6=mean(bio6),
            mean_SHM=mean(SHM),
            mean_EMT=mean(EMT),
...
# vizualisation
climDTdeem %>% pairs.panels(scale=T,hist.col="palegreen1")

climDTinit[climDTinit$prov=="CAS",] <- climDTdeem[climDTdeem$prov=="CAS",]

climDTinit <- climDTinit %>% 
  mutate(
  mean_invbio6 = mean_bio6 * (-1),
  min_invbio6 = min_bio6 * (-1))

saveRDS(climDTinit,file="data/DF_NewDriversAfterReview.rds")
```


```{r}
dfmaurizio <- read.csv("data/Climate/Raw/ClimateDTdownloads/DEEMElevationData/ClimateDT_report.csv") %>% 
  dplyr::select(ID,Latitude,Longitude,Elevation,RefElev1kmFORdownscaling) %>% 
  dplyr::rename(prov=ID,
                Elevation_original=Elevation,
                Elevation_DEEM=RefElev1kmFORdownscaling) %>% 
  merge(climDTinit[,c("prov","EMT","min_EMT","mean_EMT","SHM","mean_SHM")],by="prov") %>% 
  dplyr::rename(EMT_rast2021=EMT,
                minEMT_ext2022=min_EMT,
                meanEMT_rast2021=mean_EMT,
                SHM_rast2021=SHM,
                SHM_ext2022=mean_SHM) %>% 
  write_csv("data/Climate/Raw/ClimateDTdownloads/ClimaticVariables.csv")
```


# Save table for Supp Info

```{r TableSuppInfoDriverValues}
df.suppinfo <- df %>% 
  dplyr::select(prov, SHM, invEMT, EH.20km.PC1,EH.20km.PC2, EH.1km.PC1,EH.1km.PC2,Nb.val.20km,Nb.val.1km)
  

print(xtable(df.suppinfo, type = "latex",digits=2),
...
Create the columns with the climatic transfer distances:

```{r CreateColumnsCTD}
df <- df %>% 
  dplyr::mutate(CTD_asturias_EMT=(EMT-df.site[df.site$site=="asturias","EMT_site"] %>% pull()) %>% abs(),
                CTD_asturias_SHM=(SHM-df.site[df.site$site=="asturias","SHM_site"] %>% pull()) %>% abs(),
                CTD_bordeaux_EMT=(EMT-df.site[df.site$site=="bordeaux","EMT_site"] %>% pull()) %>% abs(),
                CTD_bordeaux_SHM=(SHM-df.site[df.site$site=="bordeaux","SHM_site"] %>% pull()) %>% abs(),
                CTD_portugal_EMT=(EMT-df.site[df.site$site=="portugal","EMT_site"] %>% pull()) %>% abs(),
                CTD_portugal_SHM=(SHM-df.site[df.site$site=="portugal","SHM_site"] %>% pull()) %>% abs())

df %>% 
  dplyr::select(prov,contains("CTD")) %>% 
  arrange(by=CTD_asturias_EMT) %>% 
  kable(digits=2) %>%
  kable_styling(font_size=11,bootstrap_options = c("striped","hover", "condensed"), full_width = F) %>%
  column_spec(1, bold = T)

df %>% 
  dplyr::select(contains("EMT"),contains("SHM"),-EMT) %>% 
  cor() %>% 
  lower.triangle() %>%
  kable(digits=3) %>%
  kable_styling(font_size=11,
