...
In this document, we calculate 6 variables standing for 6 potential drivers of the within-population genetic variation:
  ...
  - **Climate harshness** indexes $SHM$ (summer heat moisture index) and $EMT$ (extreme minimum temperature).
  
  - **Environmental heterogeneity** indexes $EH$ and $EHW$ (4 indexes).
...
p <- ggbiplot(pca,obs.scale = 1, var.scale = 1, 
         varname.size =5,alpha = 0.3,labels.size = 10,
         varname.adjust = 1.5) + 
  theme_bw() +
...
We first calculate $EH$ for each population as the variance of cell values in the 20-km radius, such as:

$$ EH = \frac{\sum_{ij}(x_{ij} - \bar{x})^2}{n-1}$$

where $i$ and $j$ are the geographical coordinates of the cells being considered, $x_{ij}$ is the value of the PC1 score at cell $[i,j]$, $\bar{x}$ is the weighted mean for the 20-km radius region. 
...
```{r VisualizationPC1, fig.height=8,fig.width=16}
...
<span style="color: red;">In the following analyses, we are going to use $EH$ at 20-km radius for both PC1 and PC2 and $EH$ at 1.6-km radius for both PC1 and PC2 too.</span>
...
$EMT$: he extreme minimum temperature over the period considered (1901-1950)
...
df %>% dplyr::select(A,D,EH.20km.PC1,EH.20km.PC2,
                     #EHW.20km.PC1,EHW.20km.PC2,
                     EH.1km.PC1,EH.1km.PC2,SHM,EMT) %>% 
  cor() %>% 
  lower.triangle() %>% 
  kable(digits=3) %>%
  kable_styling(font_size=11,
