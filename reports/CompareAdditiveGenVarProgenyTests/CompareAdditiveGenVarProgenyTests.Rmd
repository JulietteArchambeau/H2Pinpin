---
title: "Validation step with height data from a progeny test near Asturias"
...
We performed these analysis for height measured the 17/12/2007 (when the trees were 3-year old) and height measured in 2010 (when the trees were 6-year old).
...
plot_grid(hists[[1]],hists[[2]],nrow=1)
```
...
hists[[2]] <- hists[[2]] + ylab("")
hists <- plot_grid(hists[[1]],hists[[2]],nrow=1)
...
```{r HistogramWithoutZerosPopSpecific, fig.height=10, fig.width=20}
hists <- mclapply(traits,function(trait){
subdata.traits[[trait]] %>% 
    ggplot(aes(x=Trait)) + 
...
hists <- plot_grid(hists[[1]],hists[[2]],nrow=1)
ggsave(hists,
       file = paste0("figs/ExploratoryAnalyses/ValidationStep_Histograms_PopSpecific.png"),height=10,width=20)
hists
```
...
p <- mclapply(traits,ExtractParam) %>% 
  ...
  mutate(Drivers=case_when(Drivers=="SH.20km.PC1"~"SH1[20km]",
         ...
         Hyp=case_when(Drivers=="A"~"Admixture",
                       ...
                       Drivers=="var(MSP)"~"Temporal heterogeneity")) %>% 
  ...
  scale_colour_manual(labels = c("Height (3-year old)","Height (6-year old)"), 
...
p <- readRDS(file=paste0("outputs/models/",trait,"_ProgenyTestsAsturias.rds")) %>% 
  mclapply(function(driver) {
    broom::tidyMCMC(driver,pars=("sigma2_A"),
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
  ...
  bind_rows(.id="Drivers") %>% 
  mutate(Traits=trait,
         prov=rep(provs,length(selected.drivers.labels)),
...
p <- readRDS(file=paste0("outputs/models/",trait,"_ProgenyTestsAsturias.rds")) %>% 
  mclapply(function(driver) {
    broom::tidyMCMC(driver,pars=("h2_prov"),
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
