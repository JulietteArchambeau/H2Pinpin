...
# trait <- "HT07" # Height measurement in 2007 (3-year old)
  trait <- "HT10" # Height measurement in 2010 (6-year old)
if (trait=="HT10"){
 x.label.histogram= "Height measured in the progeny test of Asturias at 6-year old"
 x.legend = 0.127
} else if(trait=="HT07"){
 x.label.histogram= "Height measured in the progeny test of Asturias at 3-year old"
 x.legend = 0.386
}
...
p <- readRDS(file=paste0("outputs/models/",x,"_ProgenyTestsAsturias.rds")) %>% 
  mclapply(function(x) {
    broom::tidyMCMC(x,pars=("sigma2_A"),
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
...
ggsave(p,file=paste0("figs/PosteriorDistributions/ValidationStep_",x,"_SigmaA.png"),height=6,width=22)
...
mclapply(traits,function(x){
  ...
  bind_rows(.id="Drivers") %>% 
  mutate(Traits=x,
         prop.var=rep(c("sigma_r","sigma_block","sigma_prov","mean_sigma_fam"),8)) %>% 
  ...
  labs(color = "Potential drivers") +
  scale_x_discrete(labels=c("Families",
                            "Blocks",
                            "Populations",
                            "Residuals"
                            )) +
