...
ExtractPopSpecificParam <- function(trait){
  ...
  mclapply(function(driver) {
    broom.mixed::tidyMCMC(driver,pars=param,
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
...
# save in png
p %>% ggsave(file = paste0("figs/PosteriorDistributions/SelectedDrivers_",sub,".png"),width=width.png.selected.drivers,height=10)

# save in pdf for the final version of the manuscript
p %>% ggsave(file = paste0("figs/PosteriorDistributions/SelectedDrivers_",sub,".pdf"),width=width.png.selected.drivers,height=10)
```
...
readRDS(file=paste0("outputs/models/",x,"_",sub,".rds")) %>% 
   mclapply(function(driver) {
     broom.mixed::tidyMCMC(driver,pars=c("betaX","sigma_K","mean_sigma_clon"),
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
...
CTDbetaX <- lapply(traits, function(trait) {
  ...
  output <- lapply(names(list.stan), function(x){
    ...
    broom.mixed::tidyMCMC(mod,pars=(c("betaX")),
...
d <- readRDS(file=paste0("outputs/models/HeightGE.rds")) %>% 
  mclapply(function(x) broom.mixed::tidyMCMC(x,pars=("betaX"),
              droppars = NULL, 
              estimate.method = point.est, 
              ess = F, 
              rhat = F, 
...
p <- readRDS(file=paste0("outputs/models/HeightGE.rds")) %>% 
  mclapply(function(x) {
    broom.mixed::tidyMCMC(x,pars=("h2_prov"),
                droppars = NULL, 
                estimate.method = point.est, 
                ess = F, 
                rhat = F, 
