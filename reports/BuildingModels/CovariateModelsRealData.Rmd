...
In this document, we aim to estimate the relationship between potential drivers (namely, two admixture scores, two indexes of environmental heterogeneity and two indexes of climatic harshness) and the within-population genetic variation. 
...
```{r WhichTrait}
# For these traits, the response variable is not centered
CenterResponseVariable <- FALSE
LogResponseVariable <- FALSE
trait <- "POR_htoct12" # Height in  Fundão (Portugal) - October 2012
trait <- "BDX_htnov13" # Pierroton (France) - November 2013
trait <- "BDX_htnov18" # Pierroton (France) - November 2018 
trait <- "AST_htnov12" # Cabada (Asturias, Spain) - November 2012

# For this trait, the response variable is not centered and we used its logarithm as response variable
CenterResponseVariable <- FALSE
...
# For this trait, I centered the response variable to help convergence
CenterResponseVariable <- TRUE
LogResponseVariable <- FALSE
trait <- "d13C"        # Delta13C
trait <- "MeanBB"      # mean bud burst data over four years (2013, 2014, 2015, 2017)
trait <- "MeanDBB"     # mean duration of bud burst over three years (2014, 2015, 2017)
```

For each trait, we ran eight models, one for each potential drivers:
...
p <- mclapply(list.models, function(x) broom::tidyMCMC(x,pars=("betaX"),
              droppars = NULL, 
  ...
  bind_rows() %>% 
  mutate(Xp=names(list.models)) %>% 
  ggplot(aes(x = Xp, y = estimate)) +
  geom_linerange(aes(x = Xp, ymin = conf.low, ymax = conf.high)) +
  geom_point(size = 2) +
  ylab(TeX("Estimation of $\\beta_{X}$")) + xlab("") +
  labs(subtitle="95% credible intervals & Median",
       title=paste0("Trait: ",trait)) +
...
p
ggsave(p,file=paste0("figs/PosteriorDistributions/",trait,"_BetaX.png"),height=7,width=9)
```
...
df.betaX <- mclapply(traits,ExtractBetaX) %>% 
  bind_rows()
...
p <- df.betaX %>% 
  ...
  scale_color_manual(values=rev(c("#B2182B","#D73027","#F46D43", "#FDAE61","#5AAE61","#A6DBA0" ,"#4575B4", "#ABD9E9")),
                     labels = rev(c("Height (Fundão, 20 months)",
                                "Height (Pierroton, 25 months)",
                                "Height (Pierroton, 85 months)",
...
# Option 2
p <- df.betaX %>% 
  mutate(Hyp=case_when(Xp=="A"~"Demographic history",
                       Xp=="D"~"Demographic history",
...
#   #facet_grid(~fct_relevel(Hyp,"Demographic history","Climate harshness","Environmental heterogeneity"))
#   ylab(TeX("$\\beta_{X}$ estimates")) + xlab("") +
#   scale_color_manual(values=rev(c("#B2182B","#D73027","#F46D43", "#FDAE61","#5AAE61","#A6DBA0" ,"#4575B4", "#ABD9E9")),
#                      labels = rev(c("Height (Fundão, 20 months)",
#                                 "Height (Pierroton, 25 months)",
