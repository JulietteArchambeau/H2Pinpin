...
# remane columns
data <- data %>% dplyr::rename(AST_survdec11 = AST_survdic11,
...
data$tree[data$clon=="MAD"] <- paste0(stri_sub(data$tree[data$clon=="MAD"],1,3),
                                      1,
                                      stri_sub(data$tree[data$clon=="MAD"],4,-1))
...
dataNA <- data %>% dplyr::select(d13C,contains("BDX"),contains("POR"),contains("AST"))
...
prov.coord <- prov.coord[,c("CODE","ALTITUDE","LATITUDE","LONGITUDE")] %>%
  dplyr::rename(prov=CODE,altitude_prov=ALTITUDE,latitude_prov=LATITUDE, longitude_prov=LONGITUDE) %>%
...
AST_dec11$age <- 10
AST_dec11 <- AST_dec11 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
...
AST_nov12$age <- 21
AST_nov12 <- AST_nov12 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
AST_mar14$age <- 37
AST_mar14 <- AST_mar14 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
BDX_nov12$height <- NA
BDX_nov12$age <- 13
BDX_nov12 <- BDX_nov12 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
BDX_nov13$age <- 25
BDX_nov13 <- BDX_nov13 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
BDX_pheno13 <- BDX_pheno13 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
...
BDX_nov14$age <- 37
BDX_nov14 <- BDX_nov14 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
BDX_pheno14 <- BDX_pheno14 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                    clones522, age, bb, dbb)
...
BDX_nov15$age <- 49
BDX_nov15 <- BDX_nov15 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
BDX_pheno15 <- BDX_pheno15 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                      clones522, age, bb, dbb)
...
BDX_pheno17 <- BDX_pheno17 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                      clones522, age, bb, dbb)
...
BDX_nov18$age <- 85
BDX_nov18 <- BDX_nov18 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
POR_jan12$age <- 11
POR_jan12 <- POR_jan12 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
POR_may12$age <- 15
POR_may12 <- POR_may12 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
POR_oct12$age <- 20
POR_oct12 <- POR_oct12 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
POR_may13$age <- 27
POR_may13 <- POR_may13 %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                  clones522, age, survival, height)
...
POR_d13C$age <- NA
POR_d13C <- POR_d13C %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                      clones522, age, d13C)
...
POR_SLA$age <- NA
POR_SLA <- POR_SLA %>% dplyr::select(region, site, metap, prov, clon_original, block_original, clon, tree, block,
                                clones522, age, SLA)
...
total <- total %>% 
  mutate(obs = tree, num=NA) %>% 
  group_by(tree) %>%  
  mutate(num = 1:n()) %>% 
  unite(obs, obs, num, sep="_") %>% 
  dplyr::select(1:5,7,10,6,9,8,obs,11:ncol(total))
