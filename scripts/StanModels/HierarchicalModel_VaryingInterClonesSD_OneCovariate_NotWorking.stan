//Model 2:  model with the betaX * Xp on the mean of the sigma_clon
// Based on the simulations, this model is not working well, and therefore, we did not use it in the following analyses.
...
transformed parameters {
  ...
  sigma_clon = exp(log(mean_sigma_clon + betaX*X) - (square(sigma_K)/2) + z_log_sigma_clon*sigma_K) ;
